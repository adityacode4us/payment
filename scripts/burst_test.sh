...
echo "Sender: $SWID | Receiver: $RWID"

# Fire 30 concurrent transfers with same idempotency key
# (wallets have 0 balance, so all should be 'declined' — but identically)
...
for i in $(seq 1 30); do
  curl -s -X POST "$BASE_URL/transfers" \
    -H "$AUTH" -H "$CT" \
    -d "{\"from\":\"$SWID\",\"to\":\"$RWID\",\"amount_paise\":100,\"idempotency_key\":\"$IDEM_KEY\"}" >> "$TMPFILE" &
done
wait
...
# ========================================
echo "--- Gate 3: Conservation under contention (100 concurrent transfers) ---"
...
echo "Wallets: ${WIDS[*]}"
echo "All start at balance 0 — testing no money created from thin air"

for i in $(seq 1 100); do
  FI=$((RANDOM % 4))
  TI=$(( (FI + 1 + RANDOM % 3) % 4 ))
  K="cons_${i}_$(date +%s%N)"
  curl -s -X POST "$BASE_URL/transfers" \
    -H "$AUTH" -H "$CT" \
    -d "{\"from\":\"${WIDS[$FI]}\",\"to\":\"${WIDS[$TI]}\",\"amount_paise\":$((RANDOM % 1000 + 1)),\"idempotency_key\":\"$K\"}" > /dev/null 2>&1 &
done
wait
...
for wid in "${WIDS[@]}"; do
  ...
  if [ "$BAL" -lt 0 ]; then
    ALL_OK=false
    echo -e "$FAIL Wallet $wid has negative balance: $BAL"
  fi
  TOTAL=$((TOTAL + BAL))
done

if [ "$TOTAL" -eq 0 ] && [ "$ALL_OK" = true ]; then
  echo -e "$PASS Total balance = $TOTAL (conserved), all non-negative"
else
  echo -e "$FAIL Total=$TOTAL (expected 0), all_ok=$ALL_OK"
fi
